...
uniform sampler2DRect texture0; //First one
uniform sampler2DRect texture1; //Second one
uniform sampler2DRect texture2; //Third one
...
void main(){
  ...
  posChange += (shift * 4);
  posChangeAgain += (shift * -4);

  vec4 color;
  color.rgb = color0.rgb;
  color.a = (color1.r/2) + (color2.r/2);

  color0 = texture2DRect( texture0, pos );
  color1 = texture2DRect( texture1, posChange );
  color2 = texture2DRect( texture2, posChangeAgain );

  //Output of the shader
  gl_FragColor = (color0 + color1 + color2) / 3;
}
